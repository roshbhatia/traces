	...
	tests := map[string]struct {
		...
		"bash": {
			want: []string{
				...
				"printf '%s\\n' 'list' 'validate'",
				"':--provider') __traces_completion_filter \"$current\" < <(__traces_completion_values_2)",
		...
		"fish": {
			want: []string{
				...
				`-l provider -r -a '(__traces_completion_values_2)'`,
				`= "provider list"' -a '(__traces_completion_values_4)'`,
				`= "provider validate"' -a '(__traces_completion_values_5)'`,
		...
		"nu": {
			want: []string{
				...
				`--provider: string@"__traces_completion_values_2"`,
		...
		"zsh": {
			want: []string{
				...
				":value:__traces_completion_values_2",
